//
//  MyJobResearchViewController.swift
//  Entreprenetwork
//
...
class MyJobResearchViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        //ViewDidLoad ViewWill Apear //Add Menu remove all array before add //Configure data //
        // Do any additional setup after loading the view.
        self.objMapView.delegate = self
        self.objMapView.isMyLocationEnabled = false
        ...
        self.buttonWithdrawAmount.setAttributedTitle(underlineSeeDetail, for: .normal)
        //111 NewUI
...
//               self.tableActivity.register(UINib(nibName: "GroupTableViewCell", bundle: nil), forCellReuseIdentifier: "GroupTableViewCell")
    ...
    func getMyGroupListAPIRequestMethods(){
        var strMyGroup_debug = "Provider My Group - Start \(Date())   - Seconds \(Date().dateInMiliSeconds) -"
        ...
        APIRequestClient.shared.sendAPIRequest(requestType: .POST, queryString:kCustomerProviderGroupList , parameter: nil, isHudeShow: true, success: { (responseSuccess) in
                  if let success = responseSuccess as? [String:Any],let userInfo = success["success_data"] as? [String:Any]{
                    strMyGroup_debug += "Provider My Group - End \(Date()) - Seconds \(Date().dateInMiliSeconds) -"
...
extension MyJobResearchViewController:GMSMapViewDelegate{
        
}
extension MyJobResearchViewController:UITableViewDataSource,UITableViewDelegate,SectionHeaderViewDelegate,GroupCellDelegate{
    ...
    func buttonExpandselector(isExpand: Bool, row: Int, section: Int) {
        var sectionInfo: SectionInfo = sectionInfoArray[section] as! SectionInfo
               if var item:GroupCellInfo = sectionInfo.itemsInSection[row] as? GroupCellInfo{
                   ...
                   DispatchQueue.main.async {
                    self.tableActivity.reloadRows(at: [IndexPath.init(row: row, section: section)], with: .none) //reloadSections( IndexSet(integer: section), with: .none)
                   }
               }
    }
    ...
    func pushtoChatViewControllerWith(dict:[String:Any]){
        if let chatViewConroller = UIStoryboard.messages.instantiateViewController(withIdentifier: "ChatVC") as? ChatVC{
            ...
            if let quickbloxID = dict["quickblox_id"]{
                chatViewConroller.senderID = "\(quickbloxID)"
            }
...
extension MyJobResearchViewController:AddBankAccountWithdrawalDelegate{
