//
//  CallViewControllerNew.swift
//  Entreprenetwork
//
//  Created by khushbu on 17/12/20.
...
class CallViewControllerNew: UIViewController, UICollectionViewDelegateFlowLayout {
    //MARK: - IBOutlets
...
//                self.navigationController?.pushViewController(sharingVC, animated: true)
    ...
    private func userCell(userID: UInt) -> UserCellView? {
       guard let indexPath = userIndexPath(userID: userID),
        let cell = opponentsCollectionView.cellForItem(at: indexPath) as? UserCellView  else {
            return nil
        }
        return cell
    }
...
//            return IndexPath(row: 0, section: 0)
    ...
    func reloadContent() {
       ...
       // opponentsCollectionView.reloadData()
        lblUserName.text  = users.first?.userName
        lblState.text = ""
    }
...
extension CallViewControllerNew: LocalVideoViewDelegate {
...
extension CallViewControllerNew: QBRTCAudioSessionDelegate {
...
extension CallViewControllerNew: QBRTCClientDelegate {
    ...
    func session(_ session: QBRTCBaseSession, updatedStatsReport report: QBRTCStatsReport, forUserID userID: NSNumber) {
        ...
        if isInitiator == false && existingUser == nil {
            if let user = createConferenceUser(userID: userID.uintValue) {
               ...
               // reloadContent()
                lblUserName.text = users.first?.userName
                lblState.text = report.statsString()
            } else {
                usersDataSource?.loadUser(userID.uintValue, completion: { [weak self] (user) in
                    if let user = self?.createConferenceUser(userID: userID.uintValue) {
                        self?.users.insert(user, at: 0)
                        self!.lblUserName.text = self?.users.first?.userName
                        self!.lblState.text = report.statsString()
                       // self?.reloadContent()
                    }
                })
            }
        ...
        if user.connectionState == .connected,
           ...
           if let userIndexPath = self.userIndexPath(userID: user.userID),
            let cell = self.opponentsCollectionView.cellForItem(at: userIndexPath) as? UserCellView {
...
extension CallViewControllerNew: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CallConstant.opponentCollectionViewCellIdentifier,
                                                            for: indexPath) as? UserCellView else {
...
extension CallViewControllerNew: UICollectionViewDelegate {
...
extension CallViewControllerNew: CallKitManagerDelegate {
