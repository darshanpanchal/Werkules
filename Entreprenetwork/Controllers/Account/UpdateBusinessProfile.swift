...
class UpdateBusinessProfile: UIViewController,UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        //self.imgBusinessLogo.frame.size =  CGSize(width: self.view.frame.width, height: 400)//self.view.frame.width/2
        ...
        if let appDelegate = UIApplication.shared.delegate as? AppDelegate {
            self.arrayOfTravelTime =  appDelegate.arrayTravelTime
                ...
                if self.arrayOfTravelTime.count > 0{
                    /*
                    if let _ = self.currentTravelTime{
        ...
        if let providerDetail = self.providerDetail{
            self.configureCurrentProviderDetail(provider: providerDetail)
        }
    ...
    func configureCurrentProviderDetail(provider:ProviderDetail){
        
        self.businessRegisterParameters["provider_id"] = "\(provider.id)"
        
        self.businessLicenceParameters["business_logo"] = ""
        
        if provider.businessLogo.count > 0{
            if let imageURL = URL.init(string: provider.businessLogo){
                self.imgBusinessLogo.sd_setImage(with: imageURL, placeholderImage: UIImage.init(named: "background_update"), options: .refreshCached, context: nil)
            }
        }
        self.txtBusinessName.text = "\(provider.businessName)"
        
        self.txtMobileNumber.text = "\(provider.phone)"
        self.lblBusinessemail.text = "\(provider.email)"
        self.btnCountryCode.setTitle(provider.countryCode, for: .normal)
        self.txtBusinessAddress.text = "\(provider.address)"
        self.txtCity.text = "\(provider.city)"
        self.txtState.text = "\(provider.state)"
        self.txtZipcode.text = "\(provider.zipcode)"
        self.txtKeywords.text = "\(provider.keywordsForBusiness)"
            let keywordArray =  "\(provider.keywordsForBusiness)".components(separatedBy: ", ")
        self.arrayOfKeywordsTag.removeAll()
        for strKeywords in keywordArray{
        ...
        self.txtdescriptionOfBusienss.text = "\(provider.providerDetailDescription)"
        self.txtFldEIN.text = "\(provider.ein)"
        if let url = URL.init(string: "\(provider.businessLicense)"){
            self.txtFledBusinessLicence.text = url.lastPathComponent
        }
        
        self.businessLicenceParameters["business_license"] = ""
        if let url = URL.init(string: "\(provider.driverLicense)"){
            self.txtFLDdriwverLicence.text = url.lastPathComponent
        }
        self.businessLicenceParameters["driver_license"] = ""
        
        if let objURL = URL.init(string: "\(provider.insurance)"){
            self.txtFieldInsurance.text = "\(objURL.lastPathComponent)"
        }else{
            self.txtFieldInsurance.text = "\(provider.insurance)"
        }
        
        self.txtFldHowtoTravel.text = "\(provider.howLongWillingToTravel)"
        self.currentTravelTime = "\(provider.howLongWillingToTravel)"
        self.configureSelectedBusinessTravelTime()
    ...
    func configureBusinessTimePredefinePicker(){
        ...
        let doneButton = UIBarButtonItem(title: "Done", style: UIBarButtonItem.Style.plain, target: self, action: #selector(UpdateBusinessProfile.donetravelTimePicker))
        ...
        title.sizeToFit()
        let cancelButton = UIBarButtonItem(title:"Cancel", style: UIBarButtonItem.Style.plain, target: self, action: #selector(UpdateBusinessProfile.cancelFormDatePicker))
    ...
    @IBAction func btnCountryCodeSelector(sender:UIButton){
        ...
        let countryController = CountryPickerWithSectionViewController.presentController(on: self) { [weak self] (country: Country) in
            ...
            self.businessRegisterParameters["country_code"] = "\(country.dialingCode ?? "")"
    ...
    func popToLogInViewController(){
        for controller in self.navigationController!.viewControllers as Array {
            if controller.isKind(of: LoginVC.self) {
                if let controller = controller as? LoginVC{
...
//        guard let _ = self.driverLicenceData else {
//                   SAAlertBar.show(.error, message:"Please add driver license".localizedLowercase)
//                   return false
//              }
//        self.businessRegisterParameters["driver_license"] = self.driverLicenceParameters
        
//        guard let _ = self.businessLicenceData else {
...
//          return false
    ...
    func sendBusinessRegisterAPIRequest(){
        ...
        APIRequestClient.shared.sendAPIRequest(requestType: .POST, queryString:kProviderUpdate , parameter: self.businessRegisterParameters as [String:AnyObject], isHudeShow: true, success: { (responseSuccess) in
            if let success = responseSuccess as? [String:Any],let userInfo = success["success_data"] as? [String:Any]{
                DispatchQueue.main.async {
                    SAAlertBar.show(.error, message:"Your business profile has been updated".localizedLowercase)
...
extension UpdateBusinessProfile:UITextFieldDelegate{
...
extension UpdateBusinessProfile:UIImagePickerControllerDelegate,UINavigationControllerDelegate,CropViewControllerDelegate {
...
extension UpdateBusinessProfile:UIDocumentPickerDelegate {
...
extension UpdateBusinessProfile:UIPickerViewDelegate,UIPickerViewDataSource{
...
extension UpdateBusinessProfile:TagListViewDelegate{
